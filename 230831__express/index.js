...
// 유저가 로그인을 하면 Session ID 하나 발급!
// 서버측도 가지고 있고, 유저도 컴퓨터에 그 값을 가지고 있다.
...
passport.use(new LocalStrategy({
  ...
  db.collection('login').findOne({id : userID}, function(error, result){
    // result가 없을 경우 = 유저가 입력한 userID값과 db에 일치하는 값이 없다
    // done() => 파라미터 3개 받는다
    ...
    if(userPW == result.pw) {
      ...
      return done(null, flase, {message : '비밀번호 불일치'})
    }
  })
}))
...
// 해당 새션 데이터를 login collection에서 찾는다.
