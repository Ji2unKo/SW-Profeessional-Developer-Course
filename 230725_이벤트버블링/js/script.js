...
document.querySelector('.clost-btn').addEventListener('click',function(){
...
document.querySelector('.overlay').addEventListener('click', function(e){
  ...
  // e.target(유저가 누른요소)가 .overlay일 때만 코드 실행
  // e.stopPropagation() 잘 사용하지 않는 이유는 다른 개발자가 필요한 이벤트를 구현해둔 것 까지 막아버릴 수 있기 때문에 예상치 못한 에러를 발생시킬 확률이 높아진다.
